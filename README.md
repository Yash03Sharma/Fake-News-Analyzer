...
**spaCy cleaning** - for nrmalization, token filtering, and lemmatized text.
**SBERT embeddings** - for dense semantic representation of each article.
**Clustering on fake-news articles** - to generate two extra labels:
   - `fake_type_label` from semantic article clusters
   - `severity_label` from clustered sensationalism and distortion features
**Final supervised classifiers** - using the generated labels:
   - Logistic Regression
   - Linear SVM
   - XGBoost

The project is designed for the `True.csv` and `Fake_Parsable.csv` files uploaded files but it also works on the included demo sample.

## Project Idea 

This pipeline is intentionally **two-stage**:
...
This gives a richer output than plain binary classification:

- **Is the artcicle fake or true ?**
...
- Example style of generated labels:
 ...
 A feature bank is build from the raw title and body:
 ...
 - ALL-CAPS ratio
 - quote ratio
 - elongated-ratio ratio
 - digit ratio
 - normalized title ratio
 - normalized body length

Those features are clustered into **3 Groups** and then ranked by a compostite sensationalism proxy to map the clusters into:
...
> For the first SBERT run, internet is normally needed once so the embedding model can be downloaded and catched.
...
The quick demo is only for the code sanity checks. For real experiments, actual `True.csv` and `Fake_Parsable.csv` files are used.

## Main outputs

After a run, the ouptut directory contains:
